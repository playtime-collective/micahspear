<?php
function wpts_sc_video($atts){
	if(isset($atts['type'])){
		switch($atts['type']){
			case 'html5':
				return wpts_sc_video_html5($atts);
				break;
			case 'flash':
				return wpts_sc_video_flash($atts);
				break;
			case 'youtube':
				return wpts_sc_video_youtube($atts);
				break;
			case 'vimeo':
				return wpts_sc_video_vimeo($atts);
				break;
			case 'dailymotion':
				return wpts_sc_video_dailymotion($atts);
				break;
		}
	}
	return '';
...
add_shortcode('video', 'wpts_sc_video');

function wpts_sc_video_html5($atts){
	...
	if (!$height && !$width){
		/*$height = wpts_sc_get_option('video','html5_height');
		$width = wpts_sc_get_option('video','html5_width');*/
	}
...
<div class="video_frame_html5 video-js-box">
	<video class="video-js" width="100%" height="auto" {$poster_attribute} controls {$preload_attribute} {$autoplay_attribute}>
		{$mp4_source}
		{$webm_source}
		{$ogg_source}
...
function wpts_sc_video_flash($atts) {
	extract(shortcode_atts(array(
		'src' 	=> '',
		'width' 	=> false,
		'height' 	=> false,
	...
	if (!$height && $width) $height = intval($width * 9 / 16);
	if (!$height && !$width){
		/*$height = wpts_sc_get_option('video','flash_height');
		$width = wpts_sc_get_option('video','flash_width');*/
	}
...
function wpts_sc_video_vimeo($atts) {
	extract(shortcode_atts(array(
		'clip_id' 	=> '',
		'width' => false,
		'height' => false,
	...
	if (!$height && $width) $height = intval($width * 9 / 16);
	if (!$height && !$width){
		/*$height = wpts_sc_get_option('video','vimeo_height');
		$width = wpts_sc_get_option('video','vimeo_width');*/
	}
	if($title!='false'){
		$title = 1;
	}else{
...
function wpts_sc_video_youtube($atts, $content=null) {
	extract(shortcode_atts(array(
		'clip_id' 	=> '',
		'width' 	=> false,
		'height' 	=> false,
	...
	if (!$height && !$width){
		/*$height = wpts_sc_get_option('video','youbube_height');
		$width = wpts_sc_get_option('video','youbube_width');*/
	}
...
function wpts_sc_video_dailymotion($atts, $content=null) {
	...
	if (!$height && $width) $height = intval($width * 9 / 16);
	if (!$height && !$width){
		/*$height = wpts_sc_get_option('video','dailymotion_height');
		$width = wpts_sc_get_option('video','dailymotion_width');*/
	}
